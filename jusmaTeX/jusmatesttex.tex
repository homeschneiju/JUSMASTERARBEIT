...
\end{equation}
with 
$-1 < \pi(\tau) < 1$ \citep{Vogel.2015}. The empirical PACF can be determind by using Yule-Walker equations or the Durbin-Levinson algorithm (for a detailed description see \citep{Vogel.2015}).
...
\end{equation}
This means the value of a target variable at \textit{t} depends on a white noise process and previous white noise weighted by $\phi$. $\epsilon$ in this context are called \textit{innovations}. Note that $\mu$ may be 0 and MA-processes are stationary and causal \citep{Vogel.2015}.
...
\end{equation}
\\
Unfortunately, literature on AR(I)MAX models is scarce.\\
\cite{Intihar.2017} use ARIMAX to forecast container throughput with additional information of macro-economic indicators at the Port of Koper, whereas \cite{peter2012arima} forecast macroeconomic time series themselves with both ARIMA and ARIMAX models.\\
