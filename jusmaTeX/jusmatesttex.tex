...
\end{equation}
with 
$-1 < \pi(\tau) < 1$ \citep{Vogel.2015}. The empirical PACF can be determind by using Yule-Walker equations or the Durbin-Levinson algorithm (see for a detailed description \citep{Vogel.2015}).
...
1. \textbf{Order selection} of the AR and MA parameters \textit{p} and \textit{q}. This can be done via a visual identification of model parameter through ACF for MA- and PACF for AR-parameters, as mentioned above. \cite{Andreoni.2006b} provide a guideline for visual plot checks. Note that for ARMA models, simple plot checks are insufficient and commonly, \textit{p} and \textit{q} are chosen by minimizing an Information Criterion such as AIC or BIC. 
2. \textbf{Estimation of parameters} of the AR and MA components of the model, by Ordinary Least Squares (OLS) or Maximum Likelihood Estimation (MLE). 
3. \textbf{Model diagnostics}, or goodness of fit checks.
4. \textbf{Forecasting.}

LJUNG BOX (autocorr of residuals) AND JARQUE BERA (NV) TESTS! The forecast intervals (confidence intervals for forecasts) for ARIMA models are based on assumptions that the residuals are uncorrelated and normally distributed. If either of these assumptions does not hold, then the forecast intervals may be incorrect wiki
