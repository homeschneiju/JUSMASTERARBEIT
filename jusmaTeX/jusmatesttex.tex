...
According to \cite{Balkin.1997}, RNNs are better suited for time series data than FNNS; in fact, bad results in an RNN may hint that there is not time dependency in the data at all. However, a feedforward NN in its simplest form reduces to an $AR[p]$-model whereas an RNN may reduce to an $ARMA$-model, which is to be kept in mind when analyzing the data structure at hand.\\
...
Another type of error function next to SSE may be Mean Squared Error (MSE) \citep{Zhang.1998}. Note that independently of the training algorithm, the network might get stuck at a local optimum.
